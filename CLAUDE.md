...
- `FallingBars.tsx` uses one `InstancedMesh` with capacity = note count. `key={count}` remounts the mesh when a different-length MIDI loads (InstancedMesh capacity is fixed at construction). Off-window bars collapse to `scale 0`. **Re-strike seam:** guitar transcriptions emit long per-string sustains, so two consecutive same-MIDI notes would otherwise render as one continuous bar with no visible re-strike. A precomputed `instanceTrimTop: Uint8Array` flags any note whose next same-MIDI neighbor starts within `RESTRIKE_ADJACENCY_S = 0.15s` of its end; flagged bars shave `SEAM_SECONDS * scrollSpeed` off their `topY` in the `useFrame` loop. Seam is defined in seconds so the visual gap stays constant when the user tweaks the scroll speed slider. Isolated notes are untouched.
